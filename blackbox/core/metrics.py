...
active_workflows = Gauge(
    "blackbox_active_workflows", "Number of currently executing workflows"
)

# Cache metrics
cache_hits_total = Counter("blackbox_cache_hits_total", "Total number of cache hits")

cache_misses_total = Counter(
    "blackbox_cache_misses_total", "Total number of cache misses"
)

cache_size = Gauge("blackbox_cache_size", "Current number of items in cache")

# Error metrics
errors_total = Counter(
    "blackbox_errors_total", "Total number of errors", ["error_type", "workflow_id"]
)

# Info metric
blackbox_info = Info("blackbox_build", "Blackbox version and build information")
...
class MetricsContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            ...
            errors_total.labels(
                error_type=error_type, workflow_id=self.workflow_id
            ).inc()
...
class StepMetricsContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        workflow_step_duration_seconds.labels(
            workflow_id=self.workflow_id, step_id=self.step_id, adapter=self.adapter
        ).observe(duration)
...
def init_metrics(version: str = "1.0.0"):
    ...
    blackbox_info.info({"version": version, "python_version": "3.8+"})

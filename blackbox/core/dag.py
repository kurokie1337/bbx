...
class WorkflowDAG:
    ...
    def _build_graph(self, steps: List[Dict[str, Any]]) -> Dict[str, Dict]:
        ...
        for step in steps:
            ...
            graph[step_id] = {
                "step": step,
                "dependencies": set(depends_on) if isinstance(depends_on, list) else {depends_on} if depends_on else set(),
                "parallel": parallel,
                "dependents": set()
            }
    ...
    def get_execution_levels(self) -> List[List[str]]:
        ...
        """
        levels = []
        in_degree = {node_id: len(node["dependencies"]) for node_id, node in self.graph.items()}

...
class BBXSchemaGenerator:
    ...
    def _get_available_methods(registry: MCPRegistry) -> List[str]:
        ...
        methods.extend([
            ...
            "transform.merge", "transform.filter", "transform.map", "transform.reduce", "transform.extract", "transform.format",
            "telegram.send",
            "browser.open", "browser.goto", "browser.click", "browser.type", "browser.text", "browser.screenshot", "browser.close",
            "system.shell", "system.fs.delete", "system.fs.list"
        ])
        
        # MCP Bridge adapters (dynamic)
        # We can add common ones here
        mcp_servers = [
            "firebase", "github", "stripe", "mongodb", "notion", "linear", 
            "neon", "redis", "supabase", "prisma"
        ]

...
class MCPBridgeAdapter(MCPAdapter):
    ...
    def __init__(
        self, server_name: Optional[str] = None, config: Optional[Dict[str, Any]] = None
    ):
        """
    ...
    async def execute(self, method: str, inputs: Dict[str, Any]) -> Any:
        ...
        if not server:
            raise ValueError(
                "MCP server name is required (pass via 'mcp_server' in inputs)"
            )

        if server not in self.available_servers:
            raise ValueError(
                f"Unknown MCP server: {server}. Available: {list(self.available_servers.keys())}"
            )
    ...
    async def _execute_mcp_tool(
        ...
        mock_responses = {
            ...
            "mongodb": {
                "query": {"count": 10, "results": [{"_id": "1", "name": "Document 1"}]},
                "insert": {"inserted_id": "507f1f77bcf86cd799439011", "success": True},
            },
            "notion": {
                "list_pages": {
                    "pages": [

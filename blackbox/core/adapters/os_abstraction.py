...
class OSAbstractionAdapter(MCPAdapter):
    ...
    async def _exec(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Auto-translate Unix commands to Windows
        if auto_translate and self.is_windows:
    ...
    async def _copy(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if src_path.is_file():
                ...
                return {"copied": True, "type": "file", "src": str(src_path), "dst": str(dst_path)}
            elif src_path.is_dir():
                # Copy directory
                if recursive:
                    shutil.copytree(src_path, dst_path, dirs_exist_ok=True)
                    return {"copied": True, "type": "directory", "src": str(src_path), "dst": str(dst_path)}
                else:

...
class UniversalAdapterV2(DockerizedAdapter):
    ...
    def __init__(self, definition: Optional[Dict[str, Any]] = None):
        ...
        self.logger = logging.getLogger(f"bbx.universal.{self.definition.get('id', 'unknown')}")
    ...
    async def _execute_single(self, config: Dict[str, Any], inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_multi_step(self, steps: List[Dict], inputs: Dict[str, Any]) -> Dict[str, Any]:
        """Execute multiple steps sequentially."""
        results: List[Dict[str, Any]] = []
        context: Dict[str, Any] = {"inputs": inputs, "results": []}
        
        for i, step in enumerate(steps):
            self.logger.info(f"Executing step {i+1}/{len(steps)}: {step.get('name', 'unnamed')}")
    ...
    def _render_command(self, template: List[str], context: Dict[str, Any]) -> List[str]:
    ...
    def _parse_output(self, raw_output: Any, parser_config: Optional[Dict[str, Any]]) -> Any:
        """Parse output."""
